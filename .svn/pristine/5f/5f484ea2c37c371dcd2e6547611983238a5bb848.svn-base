...
import {
    ICourseOfDay,
    IParamsGetCourseOfDay,
    IParamsGetDriverReplacementsDrivers,
} from 'models/driverReplacements';
import React, { useEffect, useState } from 'react';
import InfiniteScroll from 'react-infinite-scroll-component';
import {
    useLazyGetCourseOfDayQuery,
    useLazyGetDriverReplacementsDriversQuery,
...
const ModalSelectCourseOfDay: React.FC<ISelectValueModal> = ({
    open,
    setOpen,
    handleSelectItem,
    driverCd,
    ...
    const [drivers, setDrivers] = useState<ICourseOfDay[]>([]);
    const [getDrivers, responseApiGet] = useLazyGetCourseOfDayQuery();
    ...
    const handleGetData = async (isSearch?) => {
        const params: IParamsGetCourseOfDay = {
            SearchText: keyword,
        ...
        <Modal className='main-menu__modal  font-inter' title={false} open={open} footer={false}>
            <div className='header flex justify-between items-center px-4 p-4 bg-green1A'>
                <div className='flex items-center gap-2'>
                    <h2 className='font-semibold text-white text-xl mb-0 mr-2'>コース一覧</h2>
                    ...
                    {responseApiGet.isLoading ? (
                        ...
                        >
                            {drivers.map((t, index) => (
                                ...
                                >
                                    <div>
                                        <span className='text-green1A text-md'>{t.courseName}</span>
                                    </div>
...
export default ModalSelectCourseOfDay;
