...
import { IParamsGetDriverReplacementsDrivers } from 'models/driverReplacements';
import React, { useEffect, useState } from 'react';
import InfiniteScroll from 'react-infinite-scroll-component';
import { useLazyGetDriverReplacementsDriversQuery } from 'services/driverReplacements';
import { useLazyGetDriversQuery } from 'services/employee';
...
const ModalSelectDriverToReplacement: React.FC<ISelectValueModal> = ({
    open,
    setOpen,
    handleSelectItem,
}) => {
    ...
    const [getDrivers, responseApiGet] = useLazyGetDriverReplacementsDriversQuery();
    ...
    const handleGetData = async (isSearch?) => {
        const params: IParamsGetDriverReplacementsDrivers = {
            SearchText: keyword,
        ...
        const response = await getDrivers(params).unwrap();
        if (response) {
            setPaging({
                ...paging,
            ...
            if (isSearch) {
                ...
                setDrivers([...drivers, ...response.items]);
            }
        }
    };
        ...
        <Modal className='main-menu__modal  font-inter' title={false} open={open} footer={false}>
            <div className='header flex justify-between items-center px-4 p-4 bg-green1A'>
                <div className='flex items-center gap-2'>
                    <h2 className='font-semibold text-white text-xl mb-0 mr-2'>運転者一覧</h2>
            ...
            <div className='px-4'>
                ...
                <div>
                    {responseApiGet.isLoading ? (
                        ...
                        <InfiniteScroll
                            dataLength={drivers.length ?? 0}
                            next={loadMoreData}
                            hasMore={drivers.length < paging?.totalRecords}
...
export default ModalSelectDriverToReplacement;
