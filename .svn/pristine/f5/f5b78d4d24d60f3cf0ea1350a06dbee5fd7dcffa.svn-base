...
import { useLazyGetUsersQuery } from 'services/chat';
...
const ModalSelectUser: React.FC<ISelectValueModal> = ({ open, setOpen, handleSelectItem }) => {
    ...
    const [getUsers, responseApiGet] = useLazyGetUsersQuery();
        ...
        <Modal className='main-menu__modal  font-inter' title={false} open={open} footer={false}>
            <div className='header flex justify-between items-center px-4 p-4 bg-green1A'>
                <div className='flex items-center gap-2'>
                    <h2 className='font-semibold text-white text-xl mb-0 mr-2'>メンバ一覧</h2>
            ...
            <div className='px-4'>
                ...
                <div id='scrollableDiv' className='overflow-auto h-[calc(100vh_-_150px)]'>
                    {responseApiGet.isLoading ? (
                        ...
                        <InfiniteScroll
                            ...
                            next={loadMoreData}
                            hasMore={users.length < paging?.totalRecords}
