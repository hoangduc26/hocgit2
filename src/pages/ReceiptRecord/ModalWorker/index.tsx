...
import { useLazyGetWorkersQuery } from 'services/employee';
...
const ModalWorker: React.FC<ISelectValueModal> = ({ open, setOpen, handleSelectItem }) => {
    ...
    const [getWorkers, responseApiGet] = useLazyGetWorkersQuery();
    ...
    const handleGetData = async (params, isSearch?) => {
        const response = await getWorkers(params).unwrap();
        if (response) {
            setPaging({
                ...paging,
            ...
            if (isSearch) {
                ...
                setWorkers([...workers, ...response.items]);
            }
        }
    };
        ...
        <Modal className='main-menu__modal  font-inter' title={false} open={open} footer={false}>
            <div className='header flex justify-between items-center px-4 p-4 bg-green1A'>
                <div className='flex items-center gap-2'>
                    <h2 className='font-semibold text-white text-xl mb-0 mr-2'>作業者一覧</h2>
            ...
            <div className='px-4'>
                ...
                <div>
                    {responseApiGet.isLoading ? (
                        ...
                        <InfiniteScroll
                            dataLength={workers.length ?? 0}
                            next={loadMoreData}
                            hasMore={workers.length < paging?.totalRecords}
