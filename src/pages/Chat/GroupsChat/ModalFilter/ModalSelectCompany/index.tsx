...
import { useLazyGetCompaniesQuery } from 'services/chat';
...
const ModalSelectCompany: React.FC<ISelectValueModal> = ({ open, setOpen, handleSelectItem }) => {
    ...
    const [getCompanys, responseApiGet] = useLazyGetCompaniesQuery();
    ...
    const handleGetData = async (params, isSearch?) => {
        const response = await getCompanys(params).unwrap();
        if (response) {
            setPaging({
                ...paging,
            ...
            if (isSearch) {
                setCompanys(response.items);
            } else {
                setCompanys([...companys, ...response.items]);
            }
        }
    };
        ...
        <Modal className='main-menu__modal  font-inter' title={false} open={open} footer={false}>
            <div className='header flex justify-between items-center px-4 p-4 bg-green1A'>
                <div className='flex items-center gap-2'>
                    <h2 className='font-semibold text-white text-xl mb-0 mr-2'>業者一覧</h2>
                ...
                <div id='scrollableDiv' className='overflow-auto'>
                    {responseApiGet.isLoading ? (
                        ...
                        <InfiniteScroll
                            ...
                            next={loadMoreData}
                            hasMore={companys.length < paging?.totalRecords}
